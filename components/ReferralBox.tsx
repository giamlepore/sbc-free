...
export function ReferralBox() {
  ...
  const fetchReferrals = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/referrals?userId=${session?.user?.id}`,
      );
  ...
  const chartData = (target: number) => ({
    datasets: [
      {
        data: [
          Math.min(referrals.length, target),
          Math.max(target - referrals.length, 0),
        ],
        ...
        <div className="mb-4">
          ...
          <div className="bg-gray-700 h-2 rounded-full mb-2">
            <div
              ...
              style={{
                width: `${Math.min((referrals.length / 40) * 100, 100)}%`,
              }}
            />
          </div>
          ...
          <div className="flex justify-around mt-6 min-h-[160px]">
            <div className="w-24 h-24">
              ...
              <p className="text-center text-gray-300 text-xs mt-3">
                <strong className="text-sm">5 indicações</strong>
                <br />
                🗄️ Módulo 4 - Banco de Dados
              </p>
            </div>
            <div className="w-24 h-24">
              <Doughnut data={chartData(15)} options={chartOptions} />
              <p className="text-center text-gray-300 text-xs mt-3">
                <strong className="text-sm">15 indicações</strong>
                <br />
                💬 Mentoria com prof. Allan
              </p>
            </div>
            <div className="w-24 h-24">
              <Doughnut data={chartData(40)} options={chartOptions} />
              <p className="text-center text-gray-300 text-xs mt-3">
                <strong className="text-sm">40 indicações</strong>
                <br />
