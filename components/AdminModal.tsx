...
export function AdminModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [isCompletionsCalendarOpen, setIsCompletionsCalendarOpen] = useState(false);
  ...
  const getCompletionsForDay = (date: Date) => {
    return users.reduce((total, user) => {
      const completionsOnDay = user.courseCompletions?.filter(completion => {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="mx-auto max-w-3xl w-full bg-gray-800 rounded-xl p-6 max-h-[90vh] flex flex-col">
            ...
            <div className="bg-gray-700 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white text-2xl font-bold">{totalCompletions}</p>
                </div>
              </div>
            </div>
          </div>
                  ...
                  {Array.from({ length: getDaysInMonth(currentDate.getFullYear(), currentDate.getMonth()) }).map((_, index) => {
                    ...
                    if (index === 0) {
                      ...
                      if (startDay > 0) {
                        ...
                        return [
                          ...emptyCells.map((_, i) => (
                            <div key={`empty-${i}`} className="h-16 bg-gray-800/50 rounded-lg" />
                          )),
                          <div
          ...
          <div className="bg-gray-700 rounded-lg p-6 mb-6">
            <div 
              className="flex items-center justify-between cursor-pointer"
              onClick={() => setIsCompletionsCalendarOpen(!isCompletionsCalendarOpen)}
            >
                      ...
                      if (startDay > 0) {
                        ...
                        return [
                          ...
                          >
                            <span className="text-gray-400 text-sm">{index + 1}</span>
                            {completionsCount > 0 && (
                              <span className="text-white font-bold">{completionsCount}</span>
                            )}
                          </div>
                        ];
                      }
        ...
        >
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="mx-auto max-w-2xl w-full bg-gray-800 rounded-xl p-6">
              <Dialog.Title className="text-xl font-bold text-white mb-4">
                Conclusões em {selectedDate.toLocaleDateString('pt-BR', { 
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric'
                })}
                ...
                {getCompletionDetails(selectedDate).map((detail, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-white font-medium">{detail.userName}</p>
                        <p className="text-gray-400 text-sm">{detail.userEmail}</p>
                      </div>
                      <span className={`
                        px-2 py-1 rounded text-xs font-medium
                        ${detail.accessLevel === 'ADMIN' ? 'bg-red-500/20 text-red-400' :
                          detail.accessLevel === 'LEAD_PLUS' ? 'bg-yellow-500/20 text-yellow-400' :
                          detail.accessLevel === 'LEAD' ? 'bg-green-500/20 text-green-400' :
                          detail.accessLevel === 'STUDENT' ? 'bg-blue-500/20 text-blue-400' :
